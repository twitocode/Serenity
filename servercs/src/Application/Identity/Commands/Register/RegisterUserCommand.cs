...
public class RegisterUserCommandHandler : IRequestHandler<RegisterUserCommand, Result<string>> {
	...
	public RegisterUserCommandHandler(ILogger<RegisterUserCommand> logger, UserManager<ApplicationUser> userManager, IJwtService jwtService, IValidator<RegisterUserCommand> validator, IDataContext dataContext) {
		this.logger = logger;
	...
	public async Task<Result<string>> Handle(RegisterUserCommand request, CancellationToken cancellationToken) {
		...
		user = new ApplicationUser {
			...
			PersonDetails = new(request.Gender, request.Pronouns),
			PasswordLock = null,
			BackgroundUrl = null,
		};
